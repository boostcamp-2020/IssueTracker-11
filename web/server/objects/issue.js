...
export default class Issue{
    constructor(item){
        ...
        this.assignees = [],
        this.labels = [],
        this.milestone = new Milestone(item.milestone_id, item.milestone_title, item.milestone_status),
    ...
    setLabel = (item) => {
        ...
        if(hasLabel === false) this.labels.push(new Label(item.label_id, item.label_name, item.label_color))
    }
    ...
    setAssignee = (item) => {
        ...
        if(hasAssignee === false) this.assignees.push(new User(item.assignee_id, item.assignee_nickname));
    }
}